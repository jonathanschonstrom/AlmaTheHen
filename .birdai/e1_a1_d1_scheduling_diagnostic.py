...
BLOCKED_CONTROL = "BLOCKED — CONTROLLED_GATE_NONDETERMINISTIC"
...
def instrument_bridge(source: str) -> str:
    ...
    insert_after(
        ...
        [
            'var e1_d1_run_id := ""',
            'var e1_d1_trace_path := ""',
            'var e1_d1_sim_tick := 0',
            'var e1_d1_gate_delay_ticks := 12',
            'var e1_d1_late_request_id := -1',
            'var e1_d1_stop_tick := -1',
            'var e1_d1_neural_seed := 20260910',
            'var e1_d1_pending_request_tick := -1',
            'var e1_d1_target_apply_tick := -1',
            'var e1_d1_sent_ticks_usec := 0',
            'var e1_d1_queued_response := {}',
            'var e1_d1_deadline_misses := 0',
            'var e1_d1_session_end_recorded := false',
        ],
        "bridge declarations",
    )
    ...
    insert_after(
        ...
        [
            '\te1_d1_run_id = OS.get_environment("BIRDAI_E1_D1_RUN_ID")',
            '\te1_d1_trace_path = OS.get_environment("BIRDAI_E1_D1_TRACE_PATH")',
            '\tvar configured_gate = OS.get_environment("BIRDAI_E1_D1_GATE_DELAY_TICKS")',
            '\tif not configured_gate.is_empty():',
            '\t\te1_d1_gate_delay_ticks = int(configured_gate)',
            '\tvar configured_late = OS.get_environment("BIRDAI_E1_D1_LATE_REQUEST_ID")',
            '\tif not configured_late.is_empty():',
            '\t\te1_d1_late_request_id = int(configured_late)',
            '\tvar configured_stop = OS.get_environment("BIRDAI_E1_D1_STOP_TICK")',
            '\tif not configured_stop.is_empty():',
            '\t\te1_d1_stop_tick = int(configured_stop)',
            '\tvar configured_seed = OS.get_environment("BIRDAI_E1_D1_NEURAL_SEED")',
            '\tif not configured_seed.is_empty():',
            '\t\te1_d1_neural_seed = int(configured_seed)',
        ],
    ...
    replace_line(
        ...
        [
            '\tlog_path = e1_d1_trace_path if not e1_d1_trace_path.is_empty() else ProjectSettings.globalize_path("res://data/" + filename)'
        ],
    ...
    replace_line(
        ...
        [
            '\t\t\t"port": PORT,',
            '\t\t\t"diagnostic_stage": "E1-A1-D1",',
            '\t\t\t"run_id": e1_d1_run_id,',
            '\t\t\t"gate_delay_ticks": e1_d1_gate_delay_ticks,',
            '\t\t\t"late_request_id": e1_d1_late_request_id,',
            '\t\t\t"stop_tick": e1_d1_stop_tick,',
            '\t\t\t"tail_request_policy": "reserve_worst_case_plus_one_tick",',
            '\t\t\t"neural_seed": e1_d1_neural_seed',
        ],
    ...
    replace_line(
        ...
        [
            '\tprocess_id = OS.create_process(python_path, PackedStringArray([script, "--port", str(PORT), "--seed", str(e1_d1_neural_seed)]), false)'
        ],
    ...
    lines[begin + 1:begin + 1] = [
        ...
        '\tvar e1_d1_worst_case_target = e1_d1_sim_tick + e1_d1_gate_delay_ticks + 1',
        '\tif e1_d1_stop_tick > 0 and e1_d1_worst_case_target > e1_d1_stop_tick:',
        '\t\treturn',
    ]
    ...
    insert_after(
        ...
        '\t\tpending = true',
        [
            '\t\te1_d1_pending_request_tick = e1_d1_sim_tick',
            '\t\te1_d1_target_apply_tick = e1_d1_pending_request_tick + e1_d1_gate_delay_ticks + (1 if request_id == e1_d1_late_request_id else 0)',
            '\t\te1_d1_sent_ticks_usec = Time.get_ticks_usec()',
            '\t\tappend_log({',
            '\t\t\t"event": "request_sent",',
            '\t\t\t"run_id": e1_d1_run_id,',
            '\t\t\t"request_id": request_id,',
            '\t\t\t"request_sim_tick": e1_d1_pending_request_tick,',
            '\t\t\t"target_apply_tick": e1_d1_target_apply_tick,',
            '\t\t\t"age": agent.age,',
    ...
    lines[block_start:block_end] = [
        ...
        '\t\t\t\tparsed["run_id"] = e1_d1_run_id',
        '\t\t\t\tparsed["receive_sim_tick"] = e1_d1_sim_tick',
        '\t\t\t\tparsed["target_apply_tick"] = e1_d1_target_apply_tick',
        '\t\t\t\tparsed["request_sent_ticks_usec"] = e1_d1_sent_ticks_usec',
        '\t\t\t\tparsed["response_ticks_usec"] = Time.get_ticks_usec()',
        '\t\t\t\tparsed["round_trip_ms"] = float(parsed["response_ticks_usec"] - e1_d1_sent_ticks_usec) / 1000.0',
        '\t\t\t\tparsed["receipt_state"] = e1_d1_state(agent)',
        '\t\t\t\te1_d1_queued_response = parsed.duplicate(true)',
    ...
    methods = [
        ...
        '\te1_d1_sim_tick = tick_value',
        '',
        'func e1_d1_state(agent) -> Dictionary:',
        '\treturn {',
        ...
        '\t\t"run_id": e1_d1_run_id,',
        '\t\t"sim_tick": tick_value,',
        '\t\t"state": e1_d1_state(agent)',
        '\t})',
        '',
        'func before_simulation_step(agent, tick_value: int) -> void:',
        '\te1_d1_sim_tick = tick_value',
        '\tif e1_d1_queued_response.is_empty():',
        '\t\treturn',
        '\tif tick_value < e1_d1_target_apply_tick:',
        '\t\treturn',
        '\tvar deadline_miss = tick_value > e1_d1_target_apply_tick',
        '\tif deadline_miss:',
        '\t\te1_d1_deadline_misses += 1',
        '\tvar decision = e1_d1_queued_response.duplicate(true)',
        '\tappend_log({',
        ...
        '\t\t"state": e1_d1_state(agent)',
        '\t})',
        '\te1_d1_queued_response = {}',
        '\te1_d1_target_apply_tick = -1',
        '\te1_d1_pending_request_tick = -1',
        '\tpending = false',
        '',
        'func record_session_end(agent, reason: String) -> void:',
        '\tif e1_d1_session_end_recorded:',
        '\t\treturn',
        '\te1_d1_session_end_recorded = true',
        '\tappend_log({',
        '\t\t"event": "session_end",',
        '\t\t"run_id": e1_d1_run_id,',
        '\t\t"termination_reason": reason,',
        '\t\t"sim_tick": e1_d1_sim_tick,',
        '\t\t"deadline_misses": e1_d1_deadline_misses,',
        '\t\t"pending": pending,',
        '\t\t"pending_request_tick": e1_d1_pending_request_tick,',
        '\t\t"target_apply_tick": e1_d1_target_apply_tick,',
        '\t\t"queued_response_remaining": not e1_d1_queued_response.is_empty(),',
        '\t\t"queued_request_id": int(e1_d1_queued_response.get("request_id", -1)) if not e1_d1_queued_response.is_empty() else -1,',
        '\t\t"state": e1_d1_state(agent)',
        '\t})',
        '',
    ]
    ...
    required = [
        ...
        '"event": "response_apply_pre"',
        '"event": "sim_state"',
        'e1_d1_late_request_id',
        '"--seed", str(e1_d1_neural_seed)',
    ]
...
def instrument_main(source: str) -> str:
    ...
    insert_after(
        ...
        [
            '\tvar configured_stop_tick = OS.get_environment("BIRDAI_E1_D1_STOP_TICK")',
            '\tif not configured_stop_tick.is_empty():',
            '\t\te1_d1_stop_tick = int(configured_stop_tick)',
        ],
        "main stop tick",
        start=begin,
        end=end,
    ...
    replace_line(
        ...
        [
            '\t\twhile accumulator >= 1.0 / 60 and (e1_d1_stop_tick < 0 or e1_d1_sim_tick < e1_d1_stop_tick):'
        ],
        "main bounded deterministic step loop",
        start=begin,
        end=end,
    )
    ...
    replace_line(
        ...
        [
            ...
            '\t\t\t\tneural.before_simulation_step(agent, e1_d1_sim_tick)',
            '\t\t\tagent.step(1.0 / 60)',
            '\t\t\tif neural != null and not smoke_mode:',
            '\t\t\t\tneural.record_sim_state(agent, e1_d1_sim_tick)',
        ],
    ...
    insert_before(
        ...
        ['\t\tneural.set_sim_tick(e1_d1_sim_tick)'],
        "main publish current sim tick",
        start=begin,
        end=end,
    )
    ...
    deterministic_stop = [
        '\tif e1_d1_stop_tick > 0 and e1_d1_sim_tick >= e1_d1_stop_tick:',
        '\t\tsave(false)',
        '\t\tif neural != null:',
...
def self_test_instrumentation() -> None:
    ...
    assert 'e1_d1_stop_tick' in main
    assert 'e1_d1_worst_case_target' in bridge
...
def run_one(
    *,
    ...
    try:
        ...
        env["BIRDAI_E1_D1_RUN_ID"] = run_id
        env["BIRDAI_E1_D1_TRACE_PATH"] = str(trace_path)
        env["BIRDAI_E1_D1_GATE_DELAY_TICKS"] = str(GATE_DELAY_TICKS)
        env["BIRDAI_E1_D1_LATE_REQUEST_ID"] = str(late_request_id)
        env["BIRDAI_E1_D1_NEURAL_SEED"] = str(NEURAL_SEED)
        env["BIRDAI_E1_D1_STOP_TICK"] = str(STOP_TICK)
...
def self_test() -> None:
    ...
    print("CONTROLLED_GATE_A_B: REQUIRED")
...
def main() -> int:
    ...
    experiment_id = (
        f"e1-a1-d1-{utc_stamp()}-{RUNTIME_COMMIT[:7]}"
    )
    ...
    print("=" * 72)
    print("E1-A1-D1 CONTROLLED RESPONSE-APPLICATION SCHEDULING DIAGNOSTIC")
    ...
    print("=" * 72)
    print("STATUS: PASS")
    print(f"E1_A1_D1_VERDICT: {comparison['verdict']}")
    print(f"CONTROL_GATE_REPLAY_EQUAL: {comparison.get('control_gate_replay_equal')}")
